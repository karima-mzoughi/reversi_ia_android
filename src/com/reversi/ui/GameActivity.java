...
public class GameActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int totalwidth = displaymetrics.widthPixels-50;
		int btnWidth=totalwidth/8;
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 8; j++)
			{
				findViewById(getResId("a" + i + j)).setOnClickListener(this);
				findViewById(getResId("a" + i + j)).getLayoutParams().width=btnWidth;
				findViewById(getResId("a" + i + j)).getLayoutParams().height=btnWidth;
			}
		// game type
